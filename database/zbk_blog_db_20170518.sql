...
(
   classify_id          bigint(13) not null comment '分类id',
   name                 varchar(20) binary comment '分类名称',
   create_time          bigint(13) comment '分类创建时间的时间戳',
...
(
   comment_id           bigint(13) not null,
   doc_id               bigint(13) comment '评论id',
...
create table doc
(
   doc_id               bigint(13) not null comment '文章id',
   title                varchar(50) binary comment '文章标题',
   doc_md               text comment '文章的mk形式，makedown语言写的',
   classify_id          bigint(13) comment '文章分类id',
   tag_id               bigint(13) comment '文章标签id',
...
create table tag
(
   tag_id               bigint(13) not null comment '标签id',
